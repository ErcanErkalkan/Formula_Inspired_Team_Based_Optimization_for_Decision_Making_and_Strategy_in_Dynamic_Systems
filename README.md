...
This is the recommended reviewer command when the generated raw metrics are already present. It rebuilds dynamic tables, portfolio tables, the graphical abstract, and the manuscript PDF without rerunning all expensive optimization experiments.
...
- `experiments/results/asoc_dynamic_eval_budget.csv`
- `experiments/results/asoc_dynamic_eval_budget_table.tex`
- `experiments/results/asoc_dynamic_budget_summary.csv`
...
- `experiments/results/asoc_dynamic_fixed_budget_table.tex`
...
The static sanity-check and appendix artifacts are:
...
The obsolete legacy names `experiments/results/summary.txt` and `experiments/results/eval_budget_summary.csv` are intentionally not listed because they are not generated by the current ASOC dynamic-report pipeline. The current corresponding files are `experiments/results/asoc_dynamic_summary.txt` and `experiments/results/asoc_dynamic_eval_budget.csv`.
...
- primary fixed-budget realized evaluation budgets in `experiments/results/asoc_dynamic_budget_summary.csv`, `experiments/results/asoc_dynamic_budget_stats.csv`, and `experiments/results/asoc_dynamic_fixed_budget_table.tex`
- auxiliary generation-matched dynamic average ranks in `experiments/results/asoc_dynamic_protocol_ranks.tex`
- auxiliary generation-matched objective-evaluation budgets in `experiments/results/asoc_dynamic_eval_budget.csv` and `experiments/results/asoc_dynamic_eval_budget_table.tex`
